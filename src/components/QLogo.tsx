import React from "react";

interface QLogoProps {
  className?: string;
  size?: number;
}

export const QLogo: React.FC<QLogoProps> = ({ className = "w-10 h-10", size }) => {
  return (
    <svg
      xmlns="http://www.w3.org/2000/svg"
      viewBox="0 0 512 512"
      width={size}
      height={size}
      className={className}
      role="img"
      aria-label="Qleading Logo"
    >
      <defs>
        {/* Soft Sepia-Mustard Background Gradient */}
        <linearGradient id="mustardSepiaBg" x1="0%" y1="0%" x2="100%" y2="100%">
          <stop offset="0%" stopColor="#F3E5AB" />
          <stop offset="50%" stopColor="#EAD896" />
          <stop offset="100%" stopColor="#DFCA7E" />
        </linearGradient>
      </defs>

      {/* Light Yellow Mustard/Sepia Squircle */}
      <rect width="512" height="512" rx="120" fill="url(#mustardSepiaBg)" />
      <rect
        x="6"
        y="6"
        width="500"
        height="500"
        rx="114"
        stroke="#CBB566"
        strokeWidth="5"
        fill="none"
        opacity="0.5"
      />

      {/* Simple Cursive Black Lowercase 'q' */}
      <g stroke="#121212" strokeWidth="32" strokeLinecap="round" strokeLinejoin="round" fill="none">
        {/* Cursive entry loop and oval counter (bowl of q) */}
        <path
          d="
            M 152 232
            C 168 184, 206 142, 252 142
            C 275 142, 288 156, 288 180
            L 288 396
            C 288 424, 314 430, 336 408
            C 356 388, 370 350, 374 324
          "
        />
        <path
          d="
            M 288 210
            C 276 170, 242 144, 198 144
            C 148 144, 114 186, 114 238
            C 114 288, 148 326, 198 326
            C 242 326, 276 296, 288 250
          "
        />
      </g>
    </svg>
  );
};



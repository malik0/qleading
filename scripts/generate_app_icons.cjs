...
// Helper function to create multi-resolution ICO file
...
async function buildAll() {
  ...
  fs.writeFileSync(path.join(publicIconsDir, 'icon.svg'), masterSvg);
  ...
  const png512 = await sharp(Buffer.from(masterSvg)).resize(512, 512).png().toBuffer();
  const png192 = await sharp(Buffer.from(masterSvg)).resize(192, 192).png().toBuffer();
  const png48 = await sharp(Buffer.from(masterSvg)).resize(48, 48).png().toBuffer();
  const png32 = await sharp(Buffer.from(masterSvg)).resize(32, 32).png().toBuffer();
  const png16 = await sharp(Buffer.from(masterSvg)).resize(16, 16).png().toBuffer();
  ...
  console.log('Successfully generated all icon and favicon assets!');
}
